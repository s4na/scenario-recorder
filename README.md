...
- 選択した文字を右クリックして、Playwrightの表示確認として記録
- `chrome.storage.local` への複数記録保存
- 保存済みシナリオ一覧の表示、削除
- シナリオごとのPlaywrightテストコードとJSONLを含むzipエクスポート
- 全シナリオのPlaywrightテストコードとJSONLを含むzipエクスポート
- 録画中の右下ステップオーバーレイ
- 操作対象の周辺コンテキスト記録
...
3. 画面上の文字を期待値にしたい場合は、文字を選択して右クリックし、「この文字を確認する」を選びます。
4. 作業中はページ右下のオーバーレイで、直近の操作とこれまでのステップを確認できます。
5. 作業が終わったら拡張のpopupに戻り、「保存」を押します。
6. 保存したシナリオは「シナリオ一覧」に追加されます。不要なシナリオは一覧から削除できます。
7. シナリオごとの「エクスポート」または「全件エクスポート」から、PlaywrightテストコードとJSONLを含むzipを保存します。
