...
- 保存済み記録一覧の表示
- 1記録ごとのPlaywrightテストコードとJSONLを含むzipエクスポート
- 全記録のPlaywrightテストコードとJSONLを含むzipエクスポート
...
4. 「停止して確認」を押します。
5. 必要なら記録名を入力して「記録を保存」を押します。未入力の場合は `yyyy-mm-dd_hh-mm-ss_host-com-path` のように、日時と開始URLから作った名前で保存されます。
6. 「エクスポート」の「この記録をzipでエクスポート」からPlaywrightテストコードとJSONLを含むzipを保存します。
7. 全記録をまとめる場合は「対象と管理」の「全記録をzipでエクスポート」からzipを保存します。
